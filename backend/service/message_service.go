...
// SetDispatchService 注入客服分派服务（main 中创建 DispatchService 后调用）
func (s *MessageService) SetDispatchService(svc *DispatchService) {
	s.dispatchService = svc
}
...
func (s *MessageService) CreateMessage(input CreateMessageInput) (*models.Message, error) {
	...
	// 2.5 转人工关键词检测 — 访客在 AI 模式下触发
	if !input.SenderIsAgent && conv.ChatMode == "ai" {
		for _, kw := range []string{"转人工", "人工客服", "人工服务", "找人工", "真人"} {
			if strings.Contains(input.Content, kw) {
				s.db.Model(&models.Conversation{}).Where("id = ?", conv.ID).Update("chat_mode", "human")
				sys := &models.Message{
					ConversationID: conv.ID, SenderIsAgent: true,
					Content: "已为您转接人工客服，请稍候…", MessageType: "system_message", ChatMode: "human",
				}
				s.messages.Create(sys)
				if s.hub != nil {
					s.hub.BroadcastMessage(conv.ID, "new_message", sys)
				}
				// 分派客服（轮询在线客服）
				if s.dispatchService != nil {
					s.dispatchService.DispatchToAgent(conv.ID)
				} else if s.hub != nil {
					s.hub.BroadcastToAllAgents("new_conversation", map[string]interface{}{"conversation_id": conv.ID, "message": "访客请求转人工"})
				}
				log.Printf("✅ 对话 %d 自动转入人工", conv.ID)
				return message, nil
			}
		}
	}
	...
	if needAIReply {
		go func() {
			...
			// 超过 5 轮 AI 回复，自动追加转人工提示
			if !aiGenFailed {
				var cnt int64
				s.db.Model(&models.Message{}).Where("conversation_id = ? AND chat_mode = ? AND sender_is_agent = ?",
					aiMessage.ConversationID, "ai", true).Count(&cnt)
				if cnt >= 5 {
					aiMessage.Content = aiMessage.Content + "\n\n---\n💡 已自动为您服务多轮。如需人工客服协助，请发送「转人工」。"
				}
			}
