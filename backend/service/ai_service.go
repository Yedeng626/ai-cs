...
请简洁、友好地回答。若无法回答，可建议用户联系人工客服。不要使用Markdown格式，回复纯文本。`, userMessage)
}
...
请基于以上内容给出简洁、准确的回答。不要使用Markdown格式，回复纯文本。`, webContext, userMessage)
}
...
4. 保持友好、专业的语气
5. 回答要简洁明了，避免冗长。
6. 不要使用 Markdown 格式（不要使用 **粗体**、*斜体*、#标题 等），回复纯文本即可`, ragContext, userMessage)
}
...
4. 保持友好、专业，回答简洁明了。
5. 不要使用 Markdown 格式（不要使用 **粗体**、*斜体*、#标题 等），回复纯文本即可。`, ragContext, userMessage)
}
