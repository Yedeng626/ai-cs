...
func main() {
	...
	vectorStoreService := rag.NewVectorStoreService(vectorStore, db)
	...
	onConnect := func(conversationID uint, isVisitor bool, visitorCount int, agentID uint) {
		if isVisitor {
			...
			// 通知分派服务：被分配的客服已接入
			if dispatchService != nil && conv.AssignedAgentID != nil && *conv.AssignedAgentID == agentID {
				dispatchService.OnAgentConnected(conversationID, agentID)
			}
		}
	}
